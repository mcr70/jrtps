 ...
 * and propagates received data to SampleListeners registered.
 * <p>
 * 
...
public class RTPSReader<T> extends Endpoint {
     ...
     * 
     * @param writerData
     *            writer to remove. If corresponding writer does not exists,
     ...
     * 
     * @param sourcePrefix
     *            GuidPrefix of the remote participant sending Data message
     * @param data
     * @param timestamp
     * @throws IOException
    ...
    void createSample(GuidPrefix sourcePrefix, Data data, Time timestamp) throws IOException {
        ...
        if (wp != null) {
            ...
            log.warn("[{}] Discarding Data from unknown writer {}, {}", getGuid().getEntityId(), sourcePrefix,
                    data.getWriterId());
        }
    }
    ...
    void onHeartbeat(GuidPrefix senderGuidPrefix, Heartbeat hb) {
        log.debug("[{}] Got Heartbeat: #{} {}-{}, F:{}, L:{} from {}", getGuid().getEntityId(), hb.getCount(),
        ...
        if (wp != null) {
            ...
            log.warn("[{}] Discarding Heartbeat from unknown writer {}, {}", getGuid().getEntityId(), senderGuidPrefix,
                    hb.getWriterId());
        }
    }
    ...
    private void sendAckNack(WriterProxy wp) {
        ...
        log.debug("[{}] Sending AckNack: #{} {}, F:{} to {}", getGuid().getEntityId(), an.getCount(),
